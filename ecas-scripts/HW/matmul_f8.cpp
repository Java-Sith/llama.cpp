...
static void matmul_f8_gemm(StreamT &a, StreamT &b, StreamT &c, const int a_rows,
...
static void matmul_f8_to_stream_a(RawDataT *a, StreamT &sa, const int rows,
...
static void matmul_f8_to_stream_b(RawDataT *a, StreamT &sa, const int rows,
                             const int cols, const int rep_rows,
                             const int rep_mats) {
#pragma HLS INLINE off
  const int tcols = cols / kPackets;
...
static void matmul_f8_from_stream(RawDataT *a, StreamT &sa, const int length) {
#pragma HLS INLINE off
 ...
 */
void matmul_f8(RawDataT *a, RawDataT *b, RawDataT *c, int a_rows, int b_cols,
            int c_cols) {
...
#pragma HLS dataflow
  matmul_f8_to_stream_a(a, stream_a, a_rows, b_cols, c_cols, 1);
  matmul_f8_to_stream_b(b, stream_b, b_cols, c_cols, 1, a_rows);
  matmul_f8_gemm(stream_a, stream_b, stream_c, a_rows, b_cols, c_cols);
  matmul_f8_from_stream(c, stream_c, c_cols * a_rows);
}
}
