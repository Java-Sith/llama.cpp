...
int main(int argc, char** argv) {
    ...
    auto bo_a = xrt::bo(device, padded_size_a * sizeof(float), matmul.group_id(0));
    auto bo_b = xrt::bo(device, padded_size_b * sizeof(float), matmul.group_id(1));
    auto bo_c = xrt::bo(device, padded_size_c * sizeof(float), matmul.group_id(2));
    ...
    START_PROFILE(kernel_execution, cynq_profiler, 1)
    bo_a.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    bo_b.sync(XCL_BO_SYNC_BO_TO_DEVICE);

    //matmul(a, b + row * COLS_B, c + row * ROWS_A, ROWS_A, COLS_A, 1);
    std::cout << "Execution of the kernel: matmul\n";
    auto run = matmul(bo_a, bo_b, bo_c, a_rows, b_cols, c_cols);
